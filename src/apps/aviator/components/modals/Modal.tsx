...
export const Modal: FC<TModalProps> = forwardRef(
  (
    ...
    const footerCss: CSSProperties = {
      background:
        footerBackgroundColor || headerBackgroundColor || HEADER_COLOR,
    };
    ...
    return (
      <div style={containerCss} ref={ref} css={modalStyles} {...props}>
        {isOpen && (
          <div style={{ position: position }} css={modalContainer}>
            ...
            <div style={bodyCss} css={bodyStyles}>
              {children}
            </div>
            {footer && (
              <footer style={footerCss} css={footerStyles}>
                <div>{footer}</div>
              </footer>
            )}
          </div>
