...
const createStore = (): Store => {
  ...
  function setAtomValue<Value>(atom: Atom<Value>, value: Value) {
    ...
    if (atom.isEqual(prevAtomState, value)) return;
  ...
  function subscribeAtom<Value>(atom: Atom<Value>, listener: Listener<Value>) {
    ...
    return () => {
      ...
      if (listeners.size === 0) listenersMap.delete(atom);
    };
  }
...
export function useAtom<T>(atom: Atom<T>): T {
  const [state, dispatch] = useReducer<Reducer<T, T>, undefined>(
    (_, next) => next,
    undefined,
    () => store.get(atom),
  );
...
export function setAtom<T>(
  atom: Atom<T>,
  value: SetStateAction<T> | typeof RESET,
) {
  let newValue: T;
